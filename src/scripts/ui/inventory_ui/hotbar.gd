...
func _update_all_slots():
	for i in range(min(9, _slot_controls.size())): _update_slot(i)

func _update_slot(index: int):
	if index < 0 or index >= _slot_controls.size(): return
	...
	if _inventory and _inventory.hotbar_items.size() > index:
		...
		if item and item.has("texture") and item.texture:
			icon.texture = item.texture; icon.show()
		else: icon.hide()
	else: icon.hide()
...
func _on_hotbar_updated(index: int):
	if index == -1: _update_all_slots()
	else: _update_slot(index)

func _on_selected_slot_changed(index: int):
	for i in range(_slot_controls.size()): _highlight_slot(i, i == index)
